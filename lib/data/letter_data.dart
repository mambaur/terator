...
String suratIzinSekolah(
  ...
  String html =
      '<p style="text-align: right; ">$dateNow</p><p>Kepada Yth.<br>Bapak/Ibu Guru Wali Kelas ${account?.educationClass ?? ''}<br>${account?.educationInstitution ?? ''}</p><p><br></p><p>Dengan Hormat,</p><p>Yang bertanda tangan dibawah ini:</p><p>Nama : ${account?.parentName ?? ''}<br>Alamat : ${account?.address ?? ''}</p><p>Orange tua/Wali murid dari:</p><p>Nama : ${account?.name ?? ''}<br>Kelas: ${account?.educationClass ?? ''}</p><p>Memberitahukan bahwa siswa tersebut tidak dapat mengikuti proses belajar di sekolah pada hari $dateNow dikarenakan <b>Sakit</b>.</p><p>Demikian Surat ini saya sampaikan. Atas perhatian Bapak/Ibu saya ucapkan terimakasih.</p><p><br></p><p style="text-align: right; ">Hormat saya,</p>$image<p style="text-align: right; ">${account?.parentName ?? ''}</p>';
